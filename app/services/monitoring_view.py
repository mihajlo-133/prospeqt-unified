...
# ---------------------------------------------------------------------------
# Table view-model — Phase 3b T1
# ---------------------------------------------------------------------------

#: Sort keys allowed for the desktop table view.
TABLE_SORT_OPTIONS: list[tuple[str, str]] = [
    ("status",        "Status"),
    ("name",          "Client"),
    ("sent_today",    "Sent"),
    ("first_touch",   "1st Touch"),
...
def normalize_table_sort(sort: str | None) -> str:
    valid = {k for k, _ in TABLE_SORT_OPTIONS}
    return sort if sort in valid else "status"


def normalize_table_dir(direction: str | None) -> str:
    return "asc" if (direction or "").lower() == "asc" else "desc"


def sort_clients_table(
    data: dict[str, dict],
    sort: str,
    direction: str = "desc",
) -> list[tuple[str, dict]]:
    """Sort clients for the desktop table view by any of the table columns.

    Default direction is `desc` for numeric columns. Name sort defaults to
    `asc`. Status sort puts red first regardless of direction (matches the
    overview card grid behavior).
    """
    items = list(data.items())
    asc = direction == "asc"

    def num_key(field: str):
        return lambda kv: (-_num(kv[1], field) if not asc else _num(kv[1], field), kv[0].lower())

    if sort == "name":
        items.sort(key=lambda kv: kv[0].lower(), reverse=not asc)
    elif sort == "sent_today":
        items.sort(key=num_key("sent_today"))
    elif sort == "first_touch":
        items.sort(key=num_key("first_touch_today"))
    elif sort == "followups":
        items.sort(key=num_key("followup_today"))
    elif sort == "reply_rate":
        items.sort(key=num_key("reply_rate_today"))
    elif sort == "replied":
        items.sort(key=num_key("replies_today"))
    elif sort == "opps":
        items.sort(key=num_key("opps_today"))
    elif sort == "bounce_rate":
        items.sort(key=num_key("bounce_rate"))
    elif sort == "not_contacted":
        items.sort(key=num_key("not_contacted"))
    elif sort == "in_progress":
        items.sort(key=num_key("in_progress"))
    else:  # status default
        items.sort(key=lambda kv: (_status_rank(kv[1]), kv[0].lower()))
    return items


def build_table_row_view(
    name: str,
    entry: dict,
    workspace_id: str | None,
    slug: str | None = None,
) -> dict[str, Any]:
    """Compute a flat view-model for one desktop-table row.

    Reads `status` directly from `entry`. EmailBison normalization happens
    here so the template stays dumb: `first_touch=0`, `followups=0`,
    `in_progress="N/A"`, no `instantly_url`.
    """
    canonical_slug = slug or name.lower().replace(" ", "_")
    status = entry.get("status") or "unknown"
    health = status if status in _HEALTH_COLORS else "gray"
    ...
    if status == "loading":
        return {
            ...
            "is_loading":  True,
            "is_error":    False,
            "status_label": _STATUS_PILL_LABELS["loading"],
            "pill_class":  "pill-m",
        }
    ...
    if status == "error":
        return {
            ...
            "is_error":    True,
            "error_msg":   entry.get("error") or "Unknown error",
            "status_label": _STATUS_PILL_LABELS["error"],
            "pill_class":  "pill-r",
        }
    ...
    sent_glyph, sent_class = _TREND_GLYPHS.get(entry.get("sent_trend") or "flat", _TREND_GLYPHS["flat"])
    reply_glyph, reply_class = _TREND_GLYPHS.get(entry.get("reply_trend") or "flat", _TREND_GLYPHS["flat"])
    opps_glyph, opps_class = _TREND_GLYPHS.get(entry.get("opp_trend") or "flat", _TREND_GLYPHS["flat"])
    ...
    return {
        ...
        "sent_today":       sent_today,
        "sent_today_fmt":   f"{sent_today:,}",
        "sent_trend_glyph": sent_glyph,
        "sent_trend_class": sent_class,
        ...
        "reply_trend_glyph": reply_glyph,
        "reply_trend_class": reply_class,

        "replied":          replies_today,
        "replied_fmt":      f"{replies_today:,}",
        "opps":             opps_today,
        "opps_fmt":         f"{opps_today:,}",
        "opps_trend_glyph": opps_glyph,
        "opps_trend_class": opps_class,
